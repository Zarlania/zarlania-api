 ...
 *
 * <p>Deliberately not a column on {@code UserEntity}: keeping proof material in its own domain
 * means a query for a user can never carry a password hash out with it, however carelessly it is
 * written. One row per account — the unique constraint on {@code user_id} is what enforces that.
 */
@Entity
@Table(name = "password_credentials")
public class PasswordCredentialEntity extends BaseEntity {
